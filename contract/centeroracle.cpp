...
ACTION centraloracle::writehash(const name owner, const checksum256 hash, const std::string reveal) {
...
ACTION centraloracle::forfeithash(name owner) {
...
//claim rewards
ACTION centraloracle::claim(name owner) {
  ...
  //if quantity symbol == VEX -> token_contract
...
//temp configuration
ACTION centraloracle::configure(globalinput g) {
  ...
  if (pitr == pairs.end()){
      ...
      pairs.emplace(_self, [&](auto& o) {
        ...
        o.proposer = _self;
        o.name = "vexidr"_n;
        o.bounty_amount = asset(0, symbol("VEX", 4));
        o.base_symbol =  symbol("VEX", 4);
        o.base_type = e_asset_type::eosio_token;
        o.base_contract = "vex.token"_n;
        o.quote_symbol = symbol("IDR", 2);
        o.quote_type = e_asset_type::fiat;
        o.quote_contract = ""_n;
        o.quoted_precision = 4;
      });

      datapointstable dstore(_self, name("vexidr"_n).value);
...
ACTION centraloracle::newbounty(name proposer, pairinput pair) {
...
//cancel a bounty
ACTION centraloracle::cancelbounty(name name, std::string reason) {
...
//vote bounty
ACTION centraloracle::votebounty(name owner, name bounty) {
...
//vote bounty
ACTION centraloracle::unvotebounty(name owner, name bounty) {
...
ACTION centraloracle::editbounty(name name, pairinput pair) {
...
//edit pair 
ACTION centraloracle::editpair(pairs pair) {
...
//delete pair 
ACTION centraloracle::deletepair(name name, std::string reason) {
...
//add custodian
ACTION centraloracle::addcustodian(name name) {
...
//remove custodian
ACTION centraloracle::delcustodian(name name) {
...
//registers a user
ACTION centraloracle::reguser(name owner) {
...
ACTION centraloracle::updateusers() {

  require_auth( _self );

  userstable users(_self, _self.value);
  voters_table vtable("vexcore"_n, name("vexcore").value);
...
//Clear all data
ACTION centraloracle::clear(name pair) {
...
ACTION centraloracle::voteabuser(const name owner, const name abuser) {
  ...
  check(total_donated > 0 || total_proxied > 0, "user must donate or proxy vote to centraloracle to vote for abusers");
...
ACTION centraloracle::migratedata() {

  require_auth(_self);

  statstable stats(name("backuporacle"), name("backuporacle").value);
  statstable _stats(_self, _self.value);

  check(_stats.begin() == _stats.end(), "stats info already exists; call clear first");

  oglobaltable global(name("backuporacle"), name("backuporacle").value);
  ...
  npairstable pairs(name("backuporacle"), name("backuporacle").value);
  ...
  while (pitr != pairs.end()) {
    ...
    statstable pstats(name("backuporacle"), pitr->name.value);
    ...
    datapointstable datapoints(name("backuporacle"), pitr->name.value);
